  ...
  module Ledgers
    module Operation
      module Mixin # rubocop:disable Metrics/ModuleLength
        module ClassMethods
          def client_class
            @client_class ||= Class.const_get("#{name.split('::')[0..2].join('::')}::Client")
          end
          ...
          def operation_method
            @operation_method ||= Util::StringHelpers.underscore(name.split('::').last).to_sym
          end
        ...
        def perform
          raise LedgerSync::Error::OperationError::PerformedOperationError.new(operation: self) if @performed
